...
if __name__ == '__main__':
    ...
    freq_limit_x = freq_limit_y = freq_limit_z = 3
    ...
    if np.isinf(snr_experiment):
        ...
        if use_latex:
            ...
            #     r'(SNR = ${snr}$dB), $\bm{{r}}_{{\text{{error}}}}=' \
            #     r'\num{{{recon_error:.2e}}}$'.format(
            #         L1=vertical_samp_sz,
            #         L2=hoizontal_samp_sz,
            #         L3=depth_samp_sz,
            #         snr=snr_experiment,
            #         recon_error=dist_err)
            title_str = \
                r'${L1}\times{L2}\times{L3}$ noisy samples, ' \
                r'$\bm{{r}}_{{\text{{error}}}}=' \
                r'\num{{{recon_error:.2e}}}$'.format(
                    L1=vertical_samp_sz,
                    L2=hoizontal_samp_sz,
                    ...
                    recon_error=dist_err)
            title_str_plotly = \
                '{L1} by {L2} by {L3} samples (SNR = {snr}dB), ' \
